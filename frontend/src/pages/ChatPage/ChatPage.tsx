...
function parseInline(text: string): React.ReactNode[] {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (match[1] !== undefined) {
      nodes.push(
        <span key={match.index} className="md-img-wrapper">
          <img
            src={match[2]}
            alt={match[1] || "image"}
            className="md-img"
          ...
          />
          {match[1] && <span className="md-img-caption">{match[1]}</span>}
        </span>,
      );
    ...
    if (/^\d+[.)]\s/.test(trimmed)) {
      ...
      while (i < lines.length && /^\d+[.)]\s/.test(lines[i].trim())) {
        ...
        while (
          ...
          i++;
          if (!sub) continue;
          const imgMatch = sub.match(/^!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)$/);
          if (imgMatch) {
            subLines.push(
              <span key={i} className="md-img-wrapper md-img-block">
                <img
                  src={imgMatch[2]}
        ...
        items.push(
          <li key={i} className="md-list-item">
            <span className="md-list-label">{parseInline(itemRaw)}</span>
            {subLines.length > 0 && <ul className="md-sub-list">{subLines}</ul>}
          </li>,
        );
      }
      elements.push(
